# coding: utf-8

"""
    Objects

    These APIs are used for defining and managing policy object configurations within Strata Cloud Manager.

    The version of the OpenAPI document: 2.0.0
    Contact: support@paloaltonetworks.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from scm_objects.models.schedules_schedule_type_recurring import SchedulesScheduleTypeRecurring

class TestSchedulesScheduleTypeRecurring(unittest.TestCase):
    """SchedulesScheduleTypeRecurring unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SchedulesScheduleTypeRecurring:
        """Test SchedulesScheduleTypeRecurring
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SchedulesScheduleTypeRecurring`
        """
        model = SchedulesScheduleTypeRecurring()
        if include_optional:
            return SchedulesScheduleTypeRecurring(
                daily = [
                    '20:07-08:20012345678910'
                    ],
                weekly = scm_objects.models.schedules_schedule_type_recurring_weekly.schedules_schedule_type_recurring_weekly(
                    friday = [
                        '20:07-08:20012345678910'
                        ], 
                    monday = [
                        '20:07-08:20012345678910'
                        ], 
                    saturday = [
                        '20:07-08:20012345678910'
                        ], 
                    sunday = [
                        '20:07-08:20012345678910'
                        ], 
                    thursday = [
                        '20:07-08:20012345678910'
                        ], 
                    tuesday = [
                        '20:07-08:20012345678910'
                        ], 
                    wednesday = [
                        '20:07-08:20012345678910'
                        ], )
            )
        else:
            return SchedulesScheduleTypeRecurring(
        )
        """

    def testSchedulesScheduleTypeRecurring(self):
        """Test SchedulesScheduleTypeRecurring"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
