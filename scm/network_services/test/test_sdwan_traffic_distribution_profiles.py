# coding: utf-8

"""
    Network Services

    These APIs are used for defining and managing network services configuration within Strata Cloud Manager.

    The version of the OpenAPI document: 2.0.0
    Contact: support@paloaltonetworks.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from scm.network_services.models.sdwan_traffic_distribution_profiles import SdwanTrafficDistributionProfiles

class TestSdwanTrafficDistributionProfiles(unittest.TestCase):
    """SdwanTrafficDistributionProfiles unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SdwanTrafficDistributionProfiles:
        """Test SdwanTrafficDistributionProfiles
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SdwanTrafficDistributionProfiles`
        """
        model = SdwanTrafficDistributionProfiles()
        if include_optional:
            return SdwanTrafficDistributionProfiles(
                device = 'My Device',
                folder = 'My Folder',
                id = '123e4567-e89b-12d3-a456-426655440000',
                link_tags = [
                    scm.network_services.models.sdwan_traffic_distribution_profiles_link_tags_inner.sdwan_traffic_distribution_profiles_link_tags_inner(
                        name = '', 
                        weight = 0, )
                    ],
                name = '',
                snippet = 'My Snippet',
                traffic_distribution = 'Best Available Path'
            )
        else:
            return SdwanTrafficDistributionProfiles(
                name = '',
        )
        """

    def testSdwanTrafficDistributionProfiles(self):
        """Test SdwanTrafficDistributionProfiles"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
