...
class ZoneProtectionProfilesNonIpProtocolProtocolInner(BaseModel):
    """
    ZoneProtectionProfilesNonIpProtocolProtocolInner
    """ # noqa: E501
    enable: Optional[StrictBool] = Field(default=None, description="Enable the Ethertype code on the list.")
    ether_type: StrictStr = Field(description="Enter an Ethertype code (protocol) preceded by 0x to indicate hexadecimal (range is 0x0000 to 0xFFFF). A list can have a maximum of 64 Ethertypes. Some sources of Ethertype codes are: * [IEEE hexadecimal Ethertype](https://www.iana.org/assignments/ieee-802-numbers/ieee-802-numbers.xhtml) * [standards.ieee.org/develop/regauth/ethertype/eth.txt](https://standards-oui.ieee.org/ethertype/eth.txt) * [www.cavebear.com/archive/cavebear/Ethernet/type.html](https://www.cavebear.com/archive/cavebear/Ethernet/type.html) ")
    name: StrictStr = Field(description="Enter the protocol name that corresponds to the Ethertype code you are adding to the list. The firewall does not verify that the protocol name matches the Ethertype code but the Ethertype code does determine the protocol filter. ")
    __properties: ClassVar[List[str]] = ["enable", "ether_type", "name"]
