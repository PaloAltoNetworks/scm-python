...
class RuleBasedMove(BaseModel):
    ...
    RuleBasedMove
    """ # noqa: E501
    destination: StrictStr = Field(description="A destination of the rule. Valid destination values are top, bottom, before and after.")
    destination_rule: Optional[StrictStr] = Field(default=None, description="A destination_rule attribute is required only if the destination value is before or after. Valid destination_rule values are existing rule UUIDs within the same container.")
    rulebase: StrictStr = Field(description="A base of a rule. Valid rulebase values are pre and post.")
