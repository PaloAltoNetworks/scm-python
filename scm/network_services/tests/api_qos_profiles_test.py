...
def test_fetch_qos_profiles(qos_profiles_api, clean_qos_profile):
    """
    Test fetching a single QoS profile by name using the fetch convenience method.
    Equivalent to Go: Test_network_services_QoSProfilesAPIService_FetchQoSProfiles
    """
    # Fetch by exact name
    fetched_obj = qos_profiles_api.fetch_qos_profiles(
        name=clean_qos_profile.name,
        folder=TARGET_FOLDER
    )

    # Verify
    assert fetched_obj is not None, f"Should have found profile '{clean_qos_profile.name}'"
    assert fetched_obj.id == clean_qos_profile.id
    assert fetched_obj.name == clean_qos_profile.name
    logger.info(f"\n[SUCCESS] fetch_qos_profiles found object: {fetched_obj.name}")

    # Test fetching non-existent profile (should return None)
    not_found = qos_profiles_api.fetch_qos_profiles(
        name="non-existent-qos-profile-xyz-12345",
        folder=TARGET_FOLDER
    )
    assert not_found is None, "Should return None for non-existent profile"
    logger.info(f"\n[SUCCESS] fetch_qos_profiles correctly returned None for non-existent profile")
