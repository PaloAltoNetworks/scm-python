...
def test_fetch_authentication_rules(auth_rules_api, clean_auth_rule):
    """
    Test fetching a single authentication rule by name using the fetch convenience method.
    Equivalent to Go: Test_identity_services_AuthenticationRulesAPIService_FetchAuthenticationRules
    """
    # Fetch by exact name (position is required for rules APIs)
    fetched_obj = auth_rules_api.fetch_authentication_rules(
        name=clean_auth_rule.name,
        folder=TARGET_FOLDER,
        position="pre"
    )
    ...
    # Verify
    assert fetched_obj is not None, f"Should have found rule '{clean_auth_rule.name}'"
    assert fetched_obj.id == clean_auth_rule.id
    assert fetched_obj.name == clean_auth_rule.name
    logger.info(f"\n[SUCCESS] fetch_authentication_rules found object: {fetched_obj.name}")

    # Test fetching non-existent rule (should return None)
    not_found = auth_rules_api.fetch_authentication_rules(
        name="non-existent-auth-rule-xyz-12345",
        folder=TARGET_FOLDER,
        position="pre"
    )
    assert not_found is None, "Should return None for non-existent rule"
    logger.info(f"\n[SUCCESS] fetch_authentication_rules correctly returned None for non-existent rule")
