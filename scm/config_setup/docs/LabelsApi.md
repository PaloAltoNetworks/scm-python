...
with scm_config_setup.ApiClient(configuration) as api_client:
    ...
    api_instance = scm_config_setup.LabelsApi(api_client)
    labels = scm_config_setup.Labels() # Labels | The `label` resource definition. (optional)
