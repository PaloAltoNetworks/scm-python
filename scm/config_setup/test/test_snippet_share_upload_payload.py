# coding: utf-8

"""
    Configuration Setup

    These APIs are used to define how Strata Cloud Manager configurations are implemented.

    The version of the OpenAPI document: 2.0.0
    Contact: support@paloaltonetworks.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from scm_config_setup.models.snippet_share_upload_payload import SnippetShareUploadPayload

class TestSnippetShareUploadPayload(unittest.TestCase):
    """SnippetShareUploadPayload unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SnippetShareUploadPayload:
        """Test SnippetShareUploadPayload
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SnippetShareUploadPayload`
        """
        model = SnippetShareUploadPayload()
        if include_optional:
            return SnippetShareUploadPayload(
                id = '',
                pause_update = True,
                validate_before_update = True
            )
        else:
            return SnippetShareUploadPayload(
                id = '',
        )
        """

    def testSnippetShareUploadPayload(self):
        """Test SnippetShareUploadPayload"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
